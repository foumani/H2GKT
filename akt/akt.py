...
class AKT(nn.Module):
    def __init__(self, n_question, n_pid, d_model, n_blocks,
        ...
        self.model = Architecture(n_question=n_question, n_blocks=n_blocks, n_heads=n_heads, dropout=dropout, d_model=d_model, d_feature=d_model / n_heads, d_ff=d_ff,  kq_same=self.kq_same, model_type=self.model_type)
    ...
    def __init__(self, n_question,  n_blocks, d_model, d_feature, d_ff, n_heads, dropout, kq_same, model_type):
        ...
        if model_type in {'akt'}:
            self.blocks_1 = nn.ModuleList([
                TransformerLayer(d_model=d_model, d_feature=d_model // n_heads, d_ff=d_ff, dropout=dropout, n_heads=n_heads, kq_same=kq_same)
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, mask, query, key, values, apply_pos=True):
        ...
        # --- FIX STARTS HERE ---
        # Generate mask directly on GPU (No CPU/NumPy sync)
        # mask arg is int: 1 (allow current) or 0 (disallow current)

        # 1. Create a matrix of ones on the device
        ones = torch.ones((1, 1, seqlen, seqlen), device=query.device)

        # 2. Get the Upper Triangular part (The "Future" we shouldn't see)
        # diagonal=mask corresponds to 'k' in numpy
        future_mask = torch.triu(ones, diagonal=mask)

        # 3. Create boolean mask: True = Visible (Past), False = Hidden (Future)
        src_mask = (future_mask == 0)
        # --- FIX ENDS HERE ---
